...
public class Warns extends Command {
    public Warns() {
        ...
        setDescription("View all the warnings against an user.");
        setUsage("<Mention User | ID> [clear] - Remove all warnings with 'clear'");
    ...
    public void run(CommandContext ctx) {
        ...
        if (memberModel == null || memberModel.getWarns().isEmpty()) {
            Utils.sendEm(
                    ctx.getEvent().getChannel(),
                    ctx.getClient().getCross() + " No warns found against that user!",
                    Utils.Embeds.ERROR
            ).queue();
            return;
        }

        ArrayList<Actions> actions = memberModel.getWarns();
        Arrays.sort(actions.toArray(Actions[]::new), (a, b) -> (int) (a.getDate() - b.getDate()));

        StringBuilder builder = new StringBuilder(
                "**" + "Warns against " + memberOptional.get().getEffectiveName() + "**\n```"
        );
